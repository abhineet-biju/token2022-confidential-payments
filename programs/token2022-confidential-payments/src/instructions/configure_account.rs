...
pub struct ConfigureAccount<'info> {
    ...
    /// CHECK: Token-2022 verifies the ZK program owner and PubkeyValidity proof context.
...
impl<'info> ConfigureAccount<'info> {
    /// Adds and configures the extension after Anchor creates or validates the ATA.
    /// The client generates the proof and AES-encrypted zero; no secret keys enter this program.
    pub fn handler(
        &mut self,
